...
namespace Core
{
	...
	void Input::Clock()
	{
		...
		if (!mmu.ReadRegisterBit(HW_FF00_P1JOYP_JOYPAD, JOYP_SELECT_DPAD))
		{
			newState = m_DPADState & 0x0F;
			stateChanged = (mmu.Read(HW_FF00_P1JOYP_JOYPAD) & 0x0F) ^ newState;
		}
		// if the buttons flag is not currently being read then its ok to update the button states
		else if (!mmu.ReadRegisterBit(HW_FF00_P1JOYP_JOYPAD, JOYP_SELECT_BUTTONS))
		{
		...
		if (stateChanged)
		{
			uint8_t currentState = mmu.Read(HW_FF00_P1JOYP_JOYPAD);

			// We only want to update the lower 4 bits of the JOYPAD register which 
			// contain the buttons that are pressed.
			mmu.Write(HW_FF00_P1JOYP_JOYPAD, (currentState & 0xF0) | newState, true);
			mmu.WriteRegisterBit(HW_FF0F_IF_INTERRUPT_FLAG, IF_JOYPAD, true);
		}
	}
