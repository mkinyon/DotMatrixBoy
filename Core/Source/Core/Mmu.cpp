...
namespace Core
{
	...
	uint8_t& Mmu::ReadFromMemoryMap(uint16_t address)
	{
	...
	void Mmu::WriteToMemoryMap(uint16_t address, uint8_t value, bool rawWrite)
	{
		// $0000-$7FFF   RES and Interrupts, cart header, cart data
		if (address >= 0x0000 && address <= 0x7FFF)
		{
	...
	bool Mmu::ReadFromMemoryMapRegister(uint16_t address, int flag)
	{
		uint8_t value = ReadFromMemoryMap(address);
		return getFlag(value, flag);
	}

	void Mmu::WriteToMemoryMapRegister(uint16_t address, int flag, bool isEnabled)
	{
		uint8_t value = ReadFromMemoryMap(address);
